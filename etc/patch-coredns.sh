...
if [ ! -z $nStart ] && [ ! -z $nEnd ]; 
then
   gsed -i "$nStart,${nEnd}d" /tmp/coredns-alias-prepatch.yaml > /dev/null
fi
...
do    
    ...
    ALIASES_ENTRIES="$ALIASES_ENTRIES$RW_RULE$H$SPACES$REGISTRY_SVC$NL"
done

IFS=

if [ ! "x$ALIASES_ENTRIES" = "x" ];
then
   # Add the rename rewrites after string health
   cat /tmp/coredns-alias-prepatch.yaml | sed "/health/a\\ 
     $ALIASES_ENTRIES" | tee /tmp/coredns-alias-patch.yaml > /dev/null
